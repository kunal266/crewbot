...
service.on('init:data', async () => {
  ...
  try {
    await api('/public/clients', 'POST', await ClientStore.findOne(Prefs.get('client.id')));
  } catch(e) {
...
// Miscellaneous APIs\
// Pop up call this method to open the selector
...
window.createAndOpenSelector = async function createAndOpenSelector(options) {
  let { url, model } = options;
  ...
  let resultSieve = await new Promise((resolve) => {
    const selector = new VisualSelector({
      loader,
      model,
    }, async function(err, sieve) {
  ...
  if(resultSieve && resultSieve.err) {
    throw new Error("Error opening Visual selector");
  } else {
    return resultSieve;
  }
};
