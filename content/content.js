...
define('api', ['id', 'underscore'], function(ID, _) {
  const MSG_INIT = 1;
  const MSG_EVENT = 2;
  const MSG_REQUEST = 3;
...
require(['jquery', 'util', 'api'], function ($, util, Api) {
  ...
  async function waitAndGetElement({selector}, {timeout = 30} = {timeout: 30}) {
  ...
  class MacroPlayer {
    ...
    async api_click({selector, pos}) {
      const el = await waitAndGetElement({selector})
      el.click()
    }

    async api_focus({selector,}) {
      const el = await waitAndGetElement({selector})
      return el.focus()
    }
    ...
    async api_scroll({selector, top, left}) {
      const el = await waitAndGetElement({selector})
      el.scrollTo({top, left});
    }

    async api_select({selector, value}) {
      const el = await waitAndGetElement({selector})
      el.value = value
    }

    async api_type({selector, value}) {
      ...
      let element
      try {
        element = await waitAndGetElement({selector})
      } catch (e) {
    ...
    async api_wait_for_element({selector}) {
      await waitAndGetElement({selector})
    }
  }
